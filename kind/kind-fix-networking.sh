...
for node in "$@"; do
    ...
    peerName=$(docker run --net=host docker.io/antrea/ethtool:latest ip link | grep ^"$peerIdx": | awk -F[:@] '{ print $2 }' | cut -c 2-)
    echo "Disabling TX checksum offload for node $node ($peerName)"
    docker run --net=host --privileged docker.io/antrea/ethtool:latest ethtool -K "$peerName" tx off
